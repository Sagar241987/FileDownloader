...
{
    if (_isText) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(30.0, 0.0, 27, 24)];
          }else{
...
- (void)setTitleText:(NSString *)text
{
    if (_titleLabel) {
        ...
        if ([text isEqualToString:@"incall"]) {
            [_imageView setImage:[UIImage imageNamed:@"persons.png"]];
        }else if ([text isEqualToString:@"chat"]){
             [_imageView setImage:[UIImage imageNamed:@"chat-66.png"]];
        }else if ([text isEqualToString:@"agenda"]){
             [_imageView setImage:[UIImage imageNamed:@"screen.png"]];
        }
    }
}
...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options withType:(BOOL)isType
{
    _isText1 = isType;
...
- (void)configureUserInterface
{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (_isText1) {
            menuItemView.isText = YES;
        }else{
           menuItemView.isText = NO;
        }
        if (_useMenuLikeSegmentedControl) {
